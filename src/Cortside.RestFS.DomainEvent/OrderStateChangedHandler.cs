...
namespace Cortside.RestFS.DomainEvent {
    /// <summary>
    ...
    public class OrderStateChangedHandler : IDomainEventHandler<OrderStateChangedEvent> {
        ...
        public async Task<HandlerResult> HandleAsync(DomainEventMessage<OrderStateChangedEvent> @event) {
            ...
            using (LogContext.PushProperty("OrderResourceId", @event.Data.OrderResourceId)) {
                logger.LogDebug("Handling {EventName} for RestFS {OrderResourceId}", typeof(OrderStateChangedEvent).Name, @event.Data.OrderResourceId);
                ...
                logger.LogDebug("Successfully handled {EventName} for RestFS {OrderResourceId}", nameof(OrderStateChangedEvent), @event.Data.OrderResourceId);
